...
public class User implements Comparable<User>, Warehouseable {
    ...
    /*public String getPassword() {
        //return new String(password, Charset.defaultCharset());
    }*/

    public boolean hasPassword(String password) {
        /*byte[] encodedPassword = this.encodePassword(password);
        for (int i = 0; i < encodedPassword.length; i++) {
            if (encodedPassword[i] != this.password[i]) {
                return false;
            }
        }
        return true;*/
    ...
    private int encodePassword(String password) {
        ...
        /*KeySpec spec = new PBEKeySpec(password.toCharArray(), new byte[]{1, 2, 3, 4, 5}, 65536, 128);
        try {
